...
def generate_occurrences_file(
    ...
    Returns two parallel lists: (raw_counts, significant_counts) per category.
    """
...
def build_latex_table(
    ...
    """Write a standalone LaTeX table summarising all PDFs."""

    n_cats = len(all_raw[0]) if all_raw else 0
    cols = "l" + "c" * n_cats + "c"  # name + categories + total

    # Category legend (C1, C2, ...) mapped to full names
    cat_legend = [f"% C{i+1}: {name}" for i, name in enumerate(category_names)]

    lines: List[str] = [
        r"% LaTeX table generated by generate_occurrences.py",
        r"% Paste this inside a table environment in your document.",
    ...
    # Header — dynamic category numbers (C1, C2, ...)
    ...
    for name, raw_row, sig_row in zip(pdf_names, all_raw, all_sig):
        ...
        cells = " & ".join(
            f"{found} / {sig} ({pct(found, sig)}\\%)"
            for found, sig in zip(raw_row, sig_row)
        )
        total_found = sum(raw_row)
        total_sig = sum(sig_row)
        total_pct = pct(total_found, total_sig)
        row = f"\\texttt{{{short_name}}} & {cells} & {total_found} / {total_sig} ({total_pct}\\%) \\\\"
        lines.append(row)
    ...
    output_path.write_text("\n".join(lines), encoding="utf-8")
    print(Fore.GREEN + f"Saved LaTeX table: {output_path}" + Style.RESET_ALL)
...
def main() -> None:
    ...
    for pdf_path in selected:
        ...
        raw_counts, sig_counts = generate_occurrences_file(pdf_path, enabler_keywords, pdf_text)
    ...
    if all_pdf_names:
        latex_path = source_folder / "summary_table.tex"
        build_latex_table(all_pdf_names, all_raw, all_sig, list(enabler_keywords.keys()), latex_path)
